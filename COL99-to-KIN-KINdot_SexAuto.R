...
path_to_input_colony99_file<-"C:\\Users\\Maxerová\\Desktop\\Misa\\colony99_example.txt"
...
for (v in secondalleles_columns)
{
  ...
  {
    if (colony_data[w,v] == "-99")
    {
      position99[w,v]<-1
      colony_data[w,v]<-colony_data[w,v-1]
    }
  }
}
...
for (y in secondalleles_columns)
{
  for (z in 1:datarow_number)
  {
    if (colony99_data[z,y] == "-99")
    {
      malematrix_k[z,3+y*2]<-"."
    }
  }
}
